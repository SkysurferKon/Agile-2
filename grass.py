...
from data.engine import scale2x
...
class GrassManager:
    ...
    def update_render(self, surf, dt, offset=(0, 0), rot_function=None):
        visible_tile_range = (int(surf.get_width() // self.tile_size) + 3, int(surf.get_height() // self.tile_size) + 3)
        ...
        for y in range(visible_tile_range[1]):
            for x in range(visible_tile_range[0]):
                pos = (base_pos[0] + (x-1), base_pos[1] + (y-1))
                if pos in self.grass_tiles:
...
class GrassAssets:
    def __init__(self, path, gm):
        ...
        for blade in sorted(os.listdir(path)):
            ...
            self.blades.append(scale2x(img))
...
class GrassTile:
    def __init__(self, tile_size, location, amt, config, ga, gm):
        ...
        for i in range(amt):
            ...
            self.blades.append([((32/amt)*i, y_pos * self.size), new_blade, random.random() * 30 - 15])
